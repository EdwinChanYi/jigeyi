...
class ShopModel(BaseModel):
    ...
    async def findByCode(self, code):
        now = int(time())
        sql = 'select * from ' + constant.MYSQL_SHOP_TABLE + ' where code = %s and status = 1 and %s >= start_time and %s <= end_time limit 1'
        row = await self.one(sql, (code, now, now), constant.MYSQL_JIGEYI_DB)
        return row